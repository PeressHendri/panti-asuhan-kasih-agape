        ...
        <div class="modal-content">
            <form action="{{ route('admin.users.update', $user->id) }}" method="POST" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    <div class="mb-3">
                        <label for="edit_name{{ $user->id }}" class="form-label">Nama <span
                    ...
                    <div class="mb-3">
                        <label for="edit_email{{ $user->id }}" class="form-label">Email <span
                                class="text-danger">*</span></label>
                    ...
                    <div class="mb-3">
                        <label for="edit_phone{{ $user->id }}" class="form-label">Nomor Handphone <span
                                class="text-danger">*</span></label>
                    ...
                    <div class="mb-3">
                        ...
                        <div class="input-group">
                            ...
                            <button class="btn btn-outline-secondary" type="button"
                                id="toggle_edit_password_{{ $user->id }}"
                                onclick="togglePassword('toggle_edit_password_{{ $user->id }}', 'edit_password{{ $user->id }}')">
                    ...
                    <div class="mb-3">
                        <label for="edit_role{{ $user->id }}" class="form-label">Role <span
                                class="text-danger">*</span></label>
                        <select name="role" id="edit_role{{ $user->id }}"
                            ...
                            <option value="pengasuh" {{ old('role', $user->role) === 'pengasuh' ? 'selected' : '' }}>
                                Pengasuh</option>
                            <option value="sponsor" {{ old('role', $user->role) === 'sponsor' ? 'selected' : '' }}>Sponsor
                                / Donatur</option>
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <input type="file" name="photo"
                            class="form-control photo-preview-input @error('photo') is-invalid @enderror"
                            data-preview-target="#edit-photo-preview-{{ $user->id }}"
                            data-preview-container="#edit-photo-preview-container-{{ $user->id }}">
                        @if($user->photo)
                            <div class="mt-2">
                                <small class="text-muted">Foto saat ini:</small><br>
                                <img src="{{ asset('storage/' . $user->photo) }}" alt="Foto" width="80"
                                    class="mt-1 rounded">
                            </div>
                        @endif
